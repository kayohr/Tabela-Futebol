// const leaderBoardAll = `SELECT
// t.team_name AS name,
// SUM(p.points) AS totalPoints,
// COUNT(*) AS totalGames,
// SUM(CASE
//   WHEN t.id = m.home_team_id AND m.home_team_goals > m.away_team_goals OR t.id = m.away_team_id
...
// LEFT JOIN TRYBE_FUTEBOL_CLUBE.matches m ON t.id = m.home_team_id OR t.id = m.away_team_id
// INNER JOIN (
// SELECT
//   m.id,
//   CASE
